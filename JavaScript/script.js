$(document).ready(function(){
	...
	var destinationData = {
		...
		wolfeIsland: {
			...
			cultureDescription: "There's even the Wolfe Island Music Festival on August 7-8 (Hayden will be there!).",
	...
	function getDestination(drivingTime, natureCulture) {
		...
		$("img").attr("src",destinationData[key].townImg).attr("alt",destinationData[key].imgAltText);
		...
		$(".link1").html(destinationData[key].linkDescription);
		...
		//Insert a string (a link description) into the <a> tag that matches the link
		$(".link2").html(destinationData[key].link2Description);
